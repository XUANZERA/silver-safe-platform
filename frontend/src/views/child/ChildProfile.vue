...
const user=ref({ name:'张小明', phone:'138****2256', relation:'张建国的子女', notify:'已开启安全提醒' })
...
<template><div class="profile-page"><header><button @click="router.push('/child')"><van-icon name="arrow-left"/></button><h1>我的信息</h1><button @click="editing=!editing">{{editing?'取消':'编辑'}}</button></header><main><section class="profile-card"><span>明</span><h2>{{user.name}}</h2><p>{{user.relation}}</p></section><section class="info-card"><van-field v-model="user.name" label="姓名" :readonly="!editing"/><van-field v-model="user.phone" label="联系电话" :readonly="!editing"/><van-field v-model="user.relation" label="关系" :readonly="!editing"/><van-cell title="安全提醒" :value="user.notify"/></section><van-button v-if="editing" block round type="primary" @click="save">保存修改</van-button><button class="logout-button" type="button" @click="logout"><van-icon name="revoke"/> 退出登录</button><p class="note">演示信息已脱敏</p></main><nav class="child-nav"><button type="button" @click="router.push('/child')"><van-icon name="home-o"/><small>首页</small></button><button type="button" @click="router.push('/schedule')"><van-icon name="todo-list-o"/><small>行程</small></button><button class="active" type="button"><van-icon name="manager-o"/><small>我的</small></button></nav></div></template>
<style scoped>
