...
const elder = ref({ name: '张建国', age: '72 岁', phone: '138****4031', family: '张小明', familyPhone: '138****2256', address: '北京市东城区（演示地址）' })
function save() { editing.value = false; showSuccessToast('信息已保存') }
function emergency() { showDialog({ title: '联系家人', message: `${elder.value.family} · ${elder.value.familyPhone}\n演示环境不会拨打真实电话。`, confirmButtonText: '知道了' }) }
async function logout() { if (isApiConfigured()) await logoutRequest().catch(() => null); userStore.logout(); router.replace('/login') }
</script>
<template>
  <div class="profile-page"><header><button type="button" @click="router.push('/elder')"><van-icon name="arrow-left" /></button><h1>我的信息</h1><button type="button" @click="editing = !editing">{{ editing ? '取消' : '编辑' }}</button></header><main><section class="profile-card"><span class="avatar">张</span><h2>{{ elder.name }}</h2><p>银发独游用户 · {{ elder.age }}</p></section><section class="info-card"><van-field v-model="elder.name" label="姓名" :readonly="!editing"/><van-field v-model="elder.age" label="年龄" :readonly="!editing"/><van-field v-model="elder.phone" label="我的电话" :readonly="!editing"/><van-field v-model="elder.family" label="家属姓名" :readonly="!editing"/><van-field v-model="elder.familyPhone" label="家属电话" :readonly="!editing"/><van-field v-model="elder.address" label="常住地址" :readonly="!editing"/></section><van-button v-if="editing" block round type="primary" @click="save">保存修改</van-button><button class="family-button" type="button" @click="emergency"><van-icon name="phone-o" /> 联系家人</button><button class="logout-button" type="button" @click="logout"><van-icon name="revoke"/> 退出登录</button><p class="note">电话和地址仅用于演示，已做脱敏处理</p></main><nav class="elder-nav"><button type="button" @click="router.push('/elder')"><van-icon name="home-o"/><small>首页</small></button><button type="button" @click="router.push('/schedule')"><van-icon name="todo-list-o"/><small>行程</small></button><button class="active" type="button"><van-icon name="manager-o"/><small>我的</small></button></nav></div>
</template>
<style scoped>
